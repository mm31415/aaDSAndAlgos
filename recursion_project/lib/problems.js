...
function fileFinder(directories, targetFile) {
  ...
  const subDirs = Object.keys(directories);

  for(let i = 0; i < subDirs.length; i++) {
    let found = false;
    const name = subDirs[i];

    if (name[0] === '/') found = fileFinder(directories[name], targetFile);
...
function pathFinder(directories, targetFile) {
  ...
  for(let i = 0; i < subDirs.length; i++) {
    const current = subDirs[i];
    let pathName = null;

    if (current[0] === '/') pathName = pathFinder(directories[current], targetFile);
    if (pathName) return current + pathName;
  }
